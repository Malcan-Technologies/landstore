...
const reviewListings = Array.from({ length: 8 }, (_, index) => ({
  id: `review-listing-${index + 1}`,
  listingSlug: `L00${index + 1}`,
  listingId: "#L001",
  submittedDate: "2024-05-20",
  submittedAgo: "3 days ago",
  ownerName: "Dato' Ridzuan",
  ownerType: "Corporate Entity",
  region: "Selangor",
  area: "Hulu Langat",
  dealTypes: ["Buy", "Financing"],
  flagState: index % 2 === 0 ? "green" : "red",
}));

const dealBadgeClass =
  "inline-flex items-center rounded-full border border-[#D6DAE3] bg-white px-2 py-1 text-[12px] font-medium text-[#52525B]";
  ...
  const rows = reviewListings.map((listing) => ({
    ...
    cells: [
      ...
      <div key="actions" className="flex items-center gap-2">
        ...
        <button
          type="button"
          className={actionButtonBase + " border border-[#D5F5E3] bg-[#EAFBF1] text-[#0F8A4A]"}
        ...
        <button
          type="button"
          className={actionButtonBase + " border border-[#FFE0E0] bg-[#FFF1F2] text-[#EF4444]"}
          aria-label="Reject listing"
        >
          <Delete size={18} className="text-[#EF4444]" />
        </button>
      </div>,
    ],
  }));
