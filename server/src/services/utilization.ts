...
/**
 * Get all utilizations with property count (paginated)
 */
export const getAllUtilizations = async (page: number = 1, limit: number = 10) => {
	...
	try {
		const [utilizations, total] = await Promise.all([
			db.utilization.findMany({
				include: {
					properties: {
						select: {
							id: true,
			...
			db.utilization.count(),
		]);

		return {
			items: utilizations.map((util: any) => ({
				id: util.id,
				name: util.name,
